...
condor_script="code/detect_events.submit"
...
for sub in sub-01 sub-02 sub-03 sub-04 sub-05 sub-06 sub-09 sub-10 sub-14 sub-15 sub-16 sub-17 sub-18 sub-19 sub-20; do
  ...
  for run in run-1 run-2 run-3 run-4 run-5 run-6 run-7 run-8; do
    ...
    echo "arguments= code/detect_events.py 30 0.0185581232561 sub-${subid}/eyegaze_run-${runid}_preprocessed.tsv.gz sub-${subid}/events_run-${runid}.tsv.gz" >> $condor_script
...
for sub in sub-22 sub-23 sub-24 sub-25 sub-26 sub-27 sub-28 sub-29 sub-30 sub-31 sub-32 sub-33 sub-34 sub-35 sub-36; do
  ...
  for run in run-1 run-2 run-3 run-4 run-5 run-6 run-7 run-8; do
    runid=$(echo "$run" | cut -d '-' -f2-)
    echo "arguments= code/detect_events.py 30 0.0266711972026 sub-${subid}/eyegaze_run-${runid}_preprocessed.tsv.gz sub-${subid}/events_run-${runid}.tsv.gz" >> $condor_script
    echo "Queue" >> $condor_script
  done
done
